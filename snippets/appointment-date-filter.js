...
function parseDateFlexible(raw) {
  ...
  // Excel serial numbers represent dates without timezone.
...
// Compute target date (tomorrow) in local operations time.
// Do not use toISOString() here: UTC conversion can shift the day around midnight.
const target = new Date();
target.setDate(target.getDate() + 1);
const targetIso = formatLocalDate(target);
