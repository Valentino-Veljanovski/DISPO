...
This layering keeps each region's business rules isolated. When one regional team changes how they assign project managers, only that region-specific sub-workflow needs to update. When DocuSign changes their notification
...
One orchestrator workflow per region. Each variant keeps region-specific business rules isolated while sharing the same overall pattern.
...
|---|---:|---|
| **Daily Duty Roster Lookup** | 15 | Read the daily duty roster from Excel, derive which crew is assigned to this booking, return the assignment to the caller. |
| **Project Manager Master Table Update** | 16 | When a new project manager appears in an incoming dispatch, append them to the master tracking table with normalized contact info. |
| **DocuSign Digital Signature Workflow** | 18 | Build the envelope payload for the booking-related document, call DocuSign API, write the resulting envelope ID into the regional tracking sheet. |
...
The reminder and daily-summary workflow is intentionally the largest part of the system and is a candidate for further decomposition. In practice it stays as one workflow because:
...
hence two workflows. The three things the reminder workflow does
