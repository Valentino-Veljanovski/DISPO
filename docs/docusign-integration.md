...
return {
  json: {
    ...
    envelopeData: {
      emailSubject: `Signature Request, ${customer.bookingId}`,
      documents: [{
...
|---|---|---|---|---|---|
| 0a1b2c... | SRL-1042 | Region A | sent | 2026-05-10T10:30:00Z | false |
...
Extract Envelope ID
        │
        ├──► Lookup Tracking Sheet (Region A)  ──┐
        ├──► Lookup Tracking Sheet (Region B)   ──┤
        └──► Lookup Tracking Sheet (Region C)  ──┤
                                                │
                                  ┌─────────────▼──────────────┐
...
e.g.
RegionA/ServiceType/Example_Customer__SRL-1042/Signed_Document.pdf
```
